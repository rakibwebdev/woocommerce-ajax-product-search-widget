    ...
    if (class_exists("Woocommerce")) {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'noResults' => esc_html__( 'No products found', 'ajax-product-search' ),
            )
        );
    }
}
...
/*-------------------*/
    ...
    function get_product_categories_hierarchy() {
        ...
        if ( ! empty( $categories ) ) {
            ...
            return new WP_Error( 'no_categories', esc_html__( 'No categories.', 'ajax-product-search' ) );
                    ...
                    $output .= '<li>';
                        $output .= '<a href="'.get_post_permalink($result->ID).'">';
                            ...
                            $output .= '<div class="product-data">';
                                ...
                                if (!empty($sku)) {
                                    $output .= '<div class="product-sku">'.esc_html__( 'SKU:', 'ajax-product-search' ).' '.$sku[0].'</div>';
                                }
    ...
    class Enovathemes_Addons_WP_Product_Search extends WP_Widget {
    	...
    	public function __construct() {
    		parent::__construct(
    			...
    			esc_html__('* Product ajax search', 'ajax-product-search'),
    			array( 'description' => esc_html__('Product ajax search', 'ajax-product-search'))
    		);
    	}
    	...
    	public function widget( $args, $instance) {
    				...
    				<form name="product-search" method="POST">
                        ...
                        <?php if ($categories): ?>
                            <select name="category" class="category">
                                <option class="default" value=""><?php echo esc_html__( 'Select a category', 'ajax-product-search' ); ?></option>
                                <?php list_taxonomy_hierarchy_no_instance( $categories); ?>
                            </select>
                        <?php endif ?>
                        <div class="search-wrapper">
                            <input type="search" name="search" class="search" placeholder="<?php echo esc_html__( 'Search for product...', 'ajax-product-search' ); ?>" value="">
     	...
     	public function form( $instance ) {
     		...
     		$defaults = array(
     			'title' => esc_html__('Product search', 'ajax-product-search'),
     		);
    		...
    		<div id="<?php echo esc_attr($this->get_field_id( 'widget_id' )); ?>">
    			...
    			<p>
    				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php echo esc_html__( 'Title:', 'ajax-product-search' ); ?></label>
